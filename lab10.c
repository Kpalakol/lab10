...
int main(int argc, char **argv) {
    ...
    time_t t1, t2;
    char o_Put48[1000], e_Prr86[1000];
    int e_Frr48, o_Fut86;
    ...
    while (fgets(line, BUFSIZ, fp1) != NULL) {
      ...
      sprintf(o_Put48, "%d", getpid());
      sprintf(e_Prr86, "%d", getpid());
      strcat(o_Put48, ".out");
      strcat(e_Prr86, ".err");
      if ((o_Fut86 = open(o_Put48, O_CREAT | O_APPEND | O_WRONLY, 0755)) == -1)
      {
        printf("Error in opening file %s \n", o_Put48);
        exit(-1);
      }
      if ((e_Frr48 = open(e_Prr86, O_CREAT | O_APPEND | O_WRONLY, 0755)) == -1)
      {
        printf("Error in  opening file %s\n", e_Prr86);
        exit(-1);
      }
      /* replace stdOut with the file <pid>.txt */
      dup2(o_Fut86, 1);
      dup2(e_Frr48, 2);
        execvp(args[0], args);
	perror("exec");
